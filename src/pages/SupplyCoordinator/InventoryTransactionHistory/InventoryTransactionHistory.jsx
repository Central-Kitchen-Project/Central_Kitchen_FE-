...
/** API sends quantity > 0 for both; direction from txType */
...
function getMovementLabel(t) {
  const tx = String(t.txType || t.TxType || '').toLowerCase();
  const typeRaw = String(
    t.transactionType || t.type || t.TransactionType || t.movementType || ''
  ).toLowerCase();

  // Out: xuất kho giao cho franchise (mọi *out* trên API)
  if (tx.includes('out') || typeRaw.includes('out')) {
    return { label: 'Stock Out (to Franchise)', tone: 'rose', code: tx };
  }
  // In: nhập kho sau khi Central duyệt request (gộp order_in, materialrequest_in, …)
  if (tx.includes('in') || typeRaw.includes('in')) {
    return { label: 'Stock In (from Central)', tone: 'emerald', code: tx };
  }
  ...
  return { label: tx || typeRaw || '—', tone: 'slate', code: tx };
}
...
function InventoryTransactionHistory() {
  ...
  const rows = useMemo(() => {
    ...
    return list.map((t) => {
      const signed = getSignedQuantity(t);
      const movement = getMovementLabel(t);
      return {
        raw: t,
        id: t.id ?? t.Id,
  ...
  /** API already scopes by userId; UI only filters by type / search */
  const filteredRows = useMemo(() => {
    let list = userId != null ? rows : [];

    if (typeFilter !== 'all') {
      list = list.filter((r) => {
        const s = r.signed;
        if (s == null) return typeFilter === 'in' ? r.movement.tone === 'emerald' : r.movement.tone === 'rose';
        return typeFilter === 'in' ? s > 0 : s < 0;
      });
    }
  ...
  const rangeLabel = useMemo(() => {
    ...
    return { from, to, total: filteredRows.length };
  ...
  return (
    <main className="flex-1 flex flex-col overflow-hidden">
      <PageHeader
        as="h2"
        title="Stock History"
        subtitle="Warehouse stock movements"
      />
      <div className="flex-1 overflow-y-auto p-8 space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-4 bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
          ...
          <div className="flex items-center gap-3">
            <select
              className="text-sm border-slate-200 rounded-lg focus:ring-primary focus:border-primary text-slate-600"
              value={typeFilter}
                ...
                {loading && filteredRows.length === 0 ? (
                  ...
                  <tr>
                    <td colSpan={6} className="px-4 py-12 text-center text-slate-400">
                      {userId == null
                        ? 'No user account. Please sign in again.'
                        : 'No transactions for this Supply account yet.'}
                    </td>
                  </tr>
                ) : (
                  paginatedRows.map((r) => (
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`inline-flex rounded-full px-2.5 py-0.5 text-xs font-bold ${
                            r.movement.tone === 'emerald'
                              ? 'bg-emerald-50 text-emerald-700'
                              : r.movement.tone === 'rose'
